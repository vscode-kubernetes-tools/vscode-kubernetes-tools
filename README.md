...
For `kubectl`, `helm` and `draft` the binaries need not to be on the system PATH. You tell to the extension their locations using the appropriate `vs-kubernetes -> vs-kubernetes.${tool}-path` configuration setting.  See [Extension Settings](#extension-settings) below.
