...
async function installToolFromTar(tool: string, urlTemplate: string, shell: Shell, supported?: Platform[]): Promise<Errorable<null>> {
    ...
    const installFolder = getInstallFolder(shell, tool);
    const executable = formatBin(tool, shell.platform())!;  // safe to assert, because if this would fail it would be caught at the OS stage
...
async function addPathToConfig(configKey: string, executableFullPath: string): Promise<void> {
    const config = vscode.workspace.getConfiguration().inspect("vs-kubernetes") || { key: "vs-kubernetes" };
...
async function addPathToConfigAtScope(configKey: string, value: string, scope: vscode.ConfigurationTarget, valueAtScope: {} | undefined, createIfNotExist: boolean): Promise<void> {
    if (!createIfNotExist) {
