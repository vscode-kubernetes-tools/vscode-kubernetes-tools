...
import { ClusterExplorerV2 } from '../../contract/cluster-explorer/v2';
...
export function impl(explorer: KubernetesExplorer): ClusterExplorerV2 {
    return new ClusterExplorerV2Impl(explorer);
}

class ClusterExplorerV2Impl implements ClusterExplorerV2 {
    ...
    get nodeSources(): ClusterExplorerV2.NodeSources {
        return {
...
function adaptToExplorerUICustomizer(nodeUICustomizer: ClusterExplorerV2.NodeUICustomizer): ExplorerUICustomizer<ClusterExplorerNode> {
...
class NodeContributorAdapter implements ExplorerExtender<ClusterExplorerNode> {
    constructor(private readonly impl: ClusterExplorerV2.NodeContributor) {}
...
class NodeUICustomizerAdapter implements ExplorerUICustomizer<ClusterExplorerNode> {
    constructor(private readonly impl: ClusterExplorerV2.NodeUICustomizer) {}
...
function adaptKubernetesExplorerNode(node: ClusterExplorerNode): ClusterExplorerV2.ClusterExplorerNode {
...
function adaptKubernetesExplorerResourceNode(node: ClusterExplorerResourceNode): ClusterExplorerV2.ClusterExplorerResourceNode {
    return {
        nodeType: 'resource',
...
export class ContributedNode implements ClusterExplorerCustomNode {
    ...
    constructor(private readonly impl: ClusterExplorerV2.Node) { }
...
function resourceFolderContributor(displayName: string, pluralDisplayName: string, manifestKind: string, abbreviation: string, apiName?: string): ClusterExplorerV2.NodeSource {
...
function groupingFolderContributor(displayName: string, contextValue: string | undefined, ...children: ClusterExplorerV2.NodeSource[]): ClusterExplorerV2.NodeSource {
...
function apiNodeSourceOf(nodeSet: NodeSourceImpl): ClusterExplorerV2.NodeSource & BuiltInNodeSource {
    return {
...
function internalNodeSourceOf(nodeSet: ClusterExplorerV2.NodeSource): NodeSourceImpl {
...
function internalNodeContributorOf(nodeContributor: ClusterExplorerV2.NodeContributor): ExplorerExtender<ClusterExplorerNode> {
...
function apiNodeContributorOf(ee: ExplorerExtender<ClusterExplorerNode>): ClusterExplorerV2.NodeContributor & BuiltInNodeContributor {
    return {
...
export function internalNodeOf(node: ClusterExplorerV2.Node): ClusterExplorerNode {
...
function apiNodeOf(node: ClusterExplorerNode): ClusterExplorerV2.Node & BuiltInNode {
    return {
