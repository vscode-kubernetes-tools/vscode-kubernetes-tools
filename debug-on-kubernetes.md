...
## 3. Extension Settings for debugging
   * `vs-kubernetes` - Parent for Kubernetes-related extension settings
      ...
      * `vs-kubernetes.nodejs-autodetect-remote-root` - This flags controlls how the extensions locates the root of the source code in the container. If true will try to automatically get the root location of the source code in the container. The detection attempts to get the CWD of the nodejs process. If false, the `remoteRoot` setting in the debug configuration will be set to `vs-kubernetes.nodejs-remote-root`.  
